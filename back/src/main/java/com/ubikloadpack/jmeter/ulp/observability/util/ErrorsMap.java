...
public class ErrorsMap {
	 ...
	 */
	public Double computeErrorTypeFrequency(String errorType, Long totalErrors) {
		if (this.errorsPerType.contains(errorType)) {
			return errorsPerType.get(errorType).computeErrorTypeFrequency(totalErrors);
		}
		return null;
	}
	 ...
	 * key was found.
	 */
	public Double computeErrorRateForType(String errorType, Long totalRequests) {
		if (this.errorsPerType.contains(errorType)) {
			return errorsPerType.get(errorType).computeErrorRate(totalRequests);
		}
		return null;
	}
	...
	public String toOpenMetric(String sampleName, Long totalThreads, Long totalErrors, Long timeStamp) {
		...
		this.errorsPerType.forEach((errorType, errorInfo) -> {
			Double errorFrequency = errorInfo.computeErrorTypeFrequency(totalErrors);
			Double errorRate = errorInfo.computeErrorRate(totalThreads);
			
			String metric = String.format("%s_total{count=\"error_every_periods\",type=\"%s\",errorRate=\"%s\",freq=\"%s\"}", sampleName, errorType, errorRate, errorFrequency);
